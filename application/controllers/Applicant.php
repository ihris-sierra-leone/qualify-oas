...
class Applicant extends CI_Controller
{
    ...
    function reject_admission()
    {
        ...
        if (!is_null($f4indexno)) {
            ...
            $ResponseData = sendXmlOverPost('http://api.tcu.go.tz/admission/reject', $xml_data);
    ...
    function rejectAdmission()
    {
        ...
        if (!is_null($f4indexno)) {
            $action = "REJECT";
            $xml_data = RejectAdmissionRequest(TCU_USERNAME, TCU_TOKEN, $f4indexno);
            $Response = sendXmlOverPost('http://api.tcu.go.tz/admission/reject', $xml_data);
    ...
    function confirmationcode()
    {
        ...
        if (!is_null($f4indexno) && !is_null($code)) {
            ...
            $ResponseData = sendXmlOverPost('http://api.tcu.go.tz/admission/confirm', $xml_data);
            $responce=RetunMessageString($ResponseData,'ResponseParameters');
            // var_dump($ResponseData);exit;
            $data = simplexml_load_string($responce);
            $json = json_encode($data);
            ...
            if($error_code !=200){
                ...
                $request_status = 1;
                $tcu_status=$this->db->query("update application set tcu_status=5,tcu_status_description='Comfirm' where id=". $this->APPLICANT->id);
    ...
    function GetComfirmationCode($applicant_id)
    {
        ...
        $xml = GetApplicantComfirmationCodeRequest(TCU_USERNAME, TCU_TOKEN,$applicant_info->form4_index,$mobile);
    ...
    function unconfirmationcode()
    {
        ...
        if (!is_null($f4indexno) && !is_null($code)) {
            ...
            $ResponseData = sendXmlOverPost('http://api.tcu.go.tz/admission/unconfirm', $xml_data);
            $responce=RetunMessageString($ResponseData,'ResponseParameters');
            // var_dump($ResponseData);exit;
            $data = simplexml_load_string($responce);
            $json = json_encode($data);
            ...
            if($error_code !=200){
                ...
                $request_status = 1;
                $tcu_status=$this->db->query("update application set tcu_status=4,tcu_status_description='Un Comfirm' where id=". $this->APPLICANT->id);
    ...
    function applicant_payment($id = null)
    {
        ...
        $this->data['invoice_info'] = $this->db->query('select * from invoices where student_id='.$this->APPLICANT->id.' and status=1 order by id DESC')->row();

        $check=$this->db->query("select * from invoices where student_id=".$this->APPLICANT->id)->row();
        if(!$check)
            //if(1==1)
        {
        ...
        $this->data['middle_content'] = 'applicant/application_fee.php';
        $this->data['content'] = 'applicant/home';
        $this->load->view('public_template', $this->data);
    }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($validate) {
                ...
                if($this->APPLICANT->application_type == 2) {
                    ...
                    //print_r($Response);
                   // echo "TCU username=".TCU_USERNAME."TCU password=".TCU_TOKEN;
                    ...
                    $response_result = $json2;
                    $insert = $this->db->query("insert into tcu_records values('','" . $this->APPLICANT->id . "','Add','$error_code','$f4index','$status','$description','$request_status','$xml_data','$response_result','$date')");
                    if ($insert) {
                        $datatoupdate = array(
                            'response' => 1,
                        );
...
//    }
    ...
    function student_invoices()
    {
                ...
                foreach ($selected_invoices as $key=>$invoice_id)
                {
                    ...
                    if($invoice_info->status==0) {
                        ...
                        $postdata = array(
                            ...
                            "GfsCode"=>GetFeeTypeDetails(3)->gfscode,
                            "amount"=>$invoice_info->amount,
                            "type"=>GetFeeTypeDetails(3)->name,
                            "secret"=>$ega_auth->api_secret,
                            "action"=>'SEND_INVOICE'
                        );
                        $url=$ega_auth->call_url;
        ...
        $this->data['middle_content'] = 'home/invoicelist';
...
//            $invoice=$this->db->query("select * from invoices where control_number=".$payment->control_number)->row();
    ...
    function student_create_invoice()
    {
        ...
        $this->form_validation->set_rules('amount', 'Invoice Amount', 'required|regex_match[/^[1-9]\d*$/]');
        if ($this->form_validation->run() == true) {
            ...
            if($code==1)
            {
                ...
                redirect('student_invoices', 'refresh');
            }else{
        ...
        $this->data['middle_content'] = 'home/createinvoice';
